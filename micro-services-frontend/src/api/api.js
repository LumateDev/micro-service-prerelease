...
export const loginUser = async (email, password) => {
  try {
    ...
    throw new Error(
      error.response?.data?.detail || "Ошибка авторизации. Попробуйте снова."
    );
  }
};
  ...
  try {
    const data = {
      ...
      date: new Date()
        ...
        .replace(/\./g, "-"), // Форматируем дату в "dd-MM-yyyy"
    };
...
export const cancelExam = async (email, examId) => {
  try {
    const response = await axios.delete(
      `${BASE_URL}/enrolments-exams/?email=${email}&exam_id=${examId}`
    );
    return response.data;
  } catch (error) {
...
export const cancelRetake = async (email, retakeId) => {
  try {
    const response = await axios.delete(
      `${BASE_URL}/enrolments-retake/?email=${email}&retake_id=${retakeId}`
    );
    return response.data;
  } catch (error) {
    throw new Error(
      "Не удалось отменить запись на пересдачу. Попробуйте снова."
    );
  }
};
...
export const fetchEnrolledExams = async (email) => {
  try {
    const response = await axios.get(
      `${BASE_URL}/enrolments-exams/?email=${email}`
    );
    return response.data;
  } catch (error) {
    throw new Error(
      "Не удалось загрузить записи на экзамены. Попробуйте снова."
    );
  }
};
...
export const fetchEnrolledRetakes = async (email) => {
  try {
    const response = await axios.get(
      `${BASE_URL}/enrolments-retake/?email=${email}`
    );
    return response.data;
  } catch (error) {
    throw new Error(
      "Не удалось загрузить записи на пересдачи. Попробуйте снова."
    );
  }
};
